...
errval_t init_ram_caps_manager(void)
{
    ...
    err = cl->rpc_tx_vtbl.get_phyaddr_cap(cl, &requested_cap, &error_code);
    assert(err_is_ok(err) && err_is_ok(error_code));

    // Initialize the memory allocator to handle PhysAddr caps
    static struct range_slot_allocator frames_allocator;
    err = range_slot_alloc_init(&frames_allocator, L2_CNODE_SLOTS, NULL);
    if (err_is_fail(err)) {
    ...
    err = mm_init(&register_manager, ObjType_RAM, ret.base, capbits, 1,
                  slab_default_refill, slot_alloc_dynamic, slot_refill_dynamic,
                  &frames_allocator, false);
    if (err_is_fail(err)) {
