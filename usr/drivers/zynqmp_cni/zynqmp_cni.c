...
static errval_t init(struct bfdriver_instance* bfi, const char* name, uint64_t
	...
	err = get_ram_cap(caps[1], SHARED_REGION_CNI_MSG_BASE, SHARED_REGION_CNI_MSG_SIZE_BITS);
	vspace_map_one_frame_attr(&va, SHARED_REGION_CNI_MSG_SIZE, caps[1], VREGION_FLAGS_READ_WRITE_NOCACHE, NULL, NULL);
